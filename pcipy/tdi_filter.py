...
class DeducedTDIFilter(LinearFilter):
    ...
    def __init__(self, measurements_data,eval_time=None,nleft=400,nright=400,in_chans=['isi'],method="simple"):
        '''
        ...
        #print('eval_time,i0:',eval_time,i0)
        ...
        print('tdi_filter: nleft, nright:',self.nleft,self.nright)
    ...
    def compute_kernel_from_measurements(input_names,measurements,i0,nleft,nright,method):
        ...
        pastwidth=len(kernel[0])
        print('pastwidth:',pastwidth)
        if method=='simple':
            pass
        elif method=='exact':
            assert ioff==0, "'exact' kernel computation does not support ioff!-0"
            kernel =  DeducedTDIFilter.compute_kernel_from_measurements_exact(input_names,measurements,i0=i0,pastwidth=pastwidth,buff=30)
        elif method=='linear':
            assert ioff==0, "'linear' kernel computation does not support ioff!-0"
            kernel =  DeducedTDIFilter.compute_kernel_from_measurements_linear(input_names,measurements,i0=i0,pastwidth=pastwidth,buff=30)
        else:
    ...
    def compute_kernel_from_measurements_exact(input_names, noise_data,i0=0,pastwidth=300,buff=0):
        '''
        ...
        buff is extra width on each side to include in the impulse response calculation
        '''

        # 1. First we restrict to a relevant stretch of data
        istart=i0-(pastwidth-1)-buff
        iend=i0+buff+pastwidth #We add pastwidth to ensure that there is room for the forward kernel from i0
    ...
    ###Now we reorganize pack that all into a neat bundle
    def compute_kernel_from_measurements_linear(input_names, noise_data,i0=0,pastwidth=300,buff=20):
        '''
        ...
        istart=i0-(pastwidth-1)-buff
        iend=i0+buff+pastwidth #We add pastwidth to ensure that there is room for the forward kernel from i0
        assert istart>=0, "out of range at start:"+str(i0)+" "+str(pastwidth)+" "+str(buff)
        ...
        # 4. Now interpolate
        #print('samp_coeffs.shape:',samp_coeffs.shape)
        #so=samp_coeffs[0]
